...
from config import HyperParams,class_nums
import numpy as np
...
class FGVC(nn.Module):
    def __init__(self, class_num, arch='resnet50'):
        ...
        self.inter = SFEM()
        self.rawcls_net = nn.Linear(2048, class_nums[HyperParams['kind']])

    def forward(self, x):
        fm1, fm2, fm3, local_logits = self.features(x)
        ...
        local_logits = self.rawcls_net(local_logits)

        return xc1, xc2, xc3, x_concat, local_logits
    ...
    def get_params(self):
        ...
        new_layers = list(self.conv_block1.parameters()) + \
                      ...
                      list(self.rawcls_net.parameters()) + \
